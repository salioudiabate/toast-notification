<div>
    @if($shown)
        <div
            x-data="initializeData()"
            x-init="setTimeout(() => { close() }, {{ ($toast['autoDismiss']) ? ($toast['duration'] != 0 ? $toast['duration'] : config('toast-notification.duration')) : config('toast-notification.longDuration') }})"
            x-show="show"
            ...
            @if (! $toast['autoDismiss'])
                <span class="absolute top-0 bottom-0 right-0 px-4 py-3" x-on:click="close()">
