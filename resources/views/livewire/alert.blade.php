<div>
    @if($shown)
        <div
            x-data="{ show: true }"
            x-init="setTimeout(() => { show = false; $wire.dismiss(); }, {{ ($toast['autoDismiss']) ? ($toast['duration'] != 0 ? $toast['duration'] : config('toast-notification.duration')) : config('toast-notification.longDuration') }})"
            x-show="show"
            ...
            @if (! $toast['autoDismiss'])
                <span x-on:click="show = false; $wire.dismiss();" class="absolute top-0 bottom-0 right-0 px-4 py-3">
