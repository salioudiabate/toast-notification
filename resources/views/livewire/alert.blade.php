...
@if($shown)
    <div
        x-data="{ show: true }"
        x-init="setTimeout(() => { show = false }, {{ ($toast['autoDismiss']) ? ($toast['duration'] != 0 ? $toast['duration'] : config('toast-notification.duration')) : config('toast-notification.longDuration') }})"
        x-show="show"
